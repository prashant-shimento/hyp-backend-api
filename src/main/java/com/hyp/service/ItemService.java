	...
	public List<Variation> getVariationsByItemId(String itemId) {
		try {
			...
			GroupOperation groupVariations = Aggregation.group("item_variations._id")
		            .first("item_variations._id").as("id")
		            .first("item_variations.name").as("name")
		            .first("item_variations.group_name").as("group_name")
		            .first("item_variations.price").as("price")
		            .first("item_variations.status").as("status")
		            .first("item_variations.active").as("active")
		            ...
		            .first("item_variations.variation_id").as("variation_id")
		            .first("item_variations.restaurant_id").as("restaurant_id")
		            .push("item_variations.addon_groups").as("addon_groups");
			...
			//TODO: Need to re-factor this
			return (List<Variation>) results.getMappedResults().stream()
		            .map(doc -> doc.get("item_variations", List.class))
		            .flatMap(List::stream)
		            .collect(Collectors.collectingAndThen(
		                Collectors.toList(),
		                this::mapToVariations 
		            ));
		} catch (Exception e) {
	...
	private List<Variation> mapToVariations(List<Document> itemVariations) {
	    return itemVariations.stream()
	        .map(variationDoc -> {
	            ...
	            List<Document> addonGroups = variationDoc.get("addon_groups", List.class);
	            if (addonGroups != null) {
