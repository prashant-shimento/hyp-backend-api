...
public class OrderService extends BaseServiceImpl<Order, String> {
	...
	public void processOrderCallback(PosCallbackRequest posCallbackRequest) {
		...
		try {
			...
			if (newOrderStatus == OrderStatusType.ACCEPTED && !order.isPreOrder()) {
				...
				order = update(order);
				int fulfillmentDelay = Optional.ofNullable(restaurant.getFulfillmentDelay()).orElse(2);
	...
	public void validatePreOrderTime(LocalDateTime preOrderDateTime, PreOrder preOrderConfig,
			List<Restaurant.DeliveryHours> deliveryHours) throws ValidationException {
		...
		Duration minLead = unit.equalsIgnoreCase("minutes") ? Duration.ofMinutes(preOrderConfig.getMinDuration())
				: Duration.ofHours(preOrderConfig.getMinDuration());

		Duration maxLead = unit.equalsIgnoreCase("minutes") ? Duration.ofMinutes(preOrderConfig.getMaxDuration())
