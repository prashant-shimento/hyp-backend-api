...
public class FeedbackService extends BaseServiceImpl<Feedback, String> {
	...
	public String constructChatIoUrl(Partner partner) {
		...
		if (accountId == null || businessNumber == null) {
			log.warn("Missing ChatIO config for partner: {} (accountId: {}, businessNumber: {})", partner.getName(),
					accountId, businessNumber);
		}
	...
	public void getDataFromChatIO(String url, Partner partner) {
		try {
			...
			if (responseData != null && responseData.getData() != null) {
				...
				log.debug("ChatIO response data size: {}", responseData.getData().size());
	...
	private String extractMessageToCheck(String message, Partner partner) {
		...
		log.debug("Extracting message to check for partner: {}. Original message: '{}', Expected welcome message: '{}'",
				partner.getName(), message, welcomeMessage);
		...
		log.warn("Message is shorter than expected welcome message for partner: {}. Returning full message: '{}'",
				partner.getName(), message);
		return message;
	}
	...
	private boolean isValidChatData(ChatData chatData, Partner partner) {
		...
		log.debug("LastMessage: '{}', LastMessageDateTimeUTC: '{}'", chatData.getLastMessage(),
	...
	private void handleFeedback(ChatData chatData, Partner partner) {
		LocalDateTime lastMessageDate = CommonUtils.getLocalDateTimeFromString(chatData.getLastMessageDateTimeUTC(),
				"yyyy-MM-dd HH:mm:ss");
		log.debug("Handling feedback for partner: {}, mobile: {}, message date: {}", partner.getName(),
				chatData.getMobileNumber(), lastMessageDate);
		if (CommonUtils.isToday(lastMessageDate.toLocalDate())) {
			log.info("Chat message is from today. Proceeding to process feedback for mobile: {}",
					chatData.getMobileNumber());
			Feedback feedback = findByMobileNumber(chatData.getMobileNumber());
			if (feedback == null) {
				log.info("No existing feedback found. Creating new feedback entry for mobile: {}",
						chatData.getMobileNumber());
	...
	private void updateExistingFeedback(ChatData chatData, Feedback feedback) {
		...
		LocalDateTime lastFeedbackAt = CommonUtils.getLocalDateTimeFromString(feedback.getLastFeedbackAt(),
				actualDateFormat);
		log.debug("Checking if existing feedback needs update. Last feedback at: {}, mobile: {}", lastFeedbackAt,
