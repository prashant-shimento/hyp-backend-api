    ...
    private void handleDeliveryExpiry(String expiredKey) {
        ...
        if (expiredKey.startsWith("delivery:")) {
            ...
            if (List.of(DeliveryFulfillStatusType.CREATED.name(), DeliveryFulfillStatusType.OUT_FOR_PICKUP.name())
                ...
                DeliveryFulfillStatusType currentStatus =
                ...
                if (currentStatus.name().equals(status)) {
                    ...
                    String triggerFulfillOnRiderDelay = redisService
                            .getRedisData(Constants.REDIS_KEY_TRIGGER_FULFILL_ON_RIDER_DELAY)
                            .orElse("false");
                    ...
                    if (triggerFulfillOnRiderDelay.equalsIgnoreCase("true")) {
                        log.info(
                                "Smart fulfilling the order again due to delay in assigning rider for order, {}",
                                orderId);
                        try {
        ...
        if (expiredKey.startsWith("rider_location:")) {
            try {
                ...
                log.info(
                        "Received Rider Location Expiry from Redis for Order ID: {} with Status: {}",
                        orderId,
                        lastStatus);
                Order order = orderService.findById(orderId);
                Delivery delivery = deliveryService.findByOrderId(orderId);
                ...
                Optional<Location> lastKnownLocation = delivery.getFulfillment().getLogs().stream()
                        .filter(log -> log.getStatus().equals(lastStatus))
                        .map(Log::getLocation)
                ...
                if (isLocationMissing) {
                    log.warn(
                            "Rider location is missing or incomplete for Order ID: {}. Setting fallback expiry.",
                            orderId);
                    ...
                    if (lastKnownLocation.isPresent()) {
                        ...
                        if (Double.compare(lastLocation.getLatitude(), currentLat) == 0
                            ...
                            notificationService.sendInternalGroupNotification(
                                    Constants.META_RIDER_DELAY_ALERT_TEMPLATE, parameters);
                            oneSignalAlertService.notifyRiderNotMovingAlert(
                                    orderId,
                                    restaurant.getRestaurantName(),
                                    delivery.getFulfillment().getStatus(),
                                    customer.getName(),
                    ...
                    if (List.of(
                                    DeliveryFulfillStatusType.OUT_FOR_PICKUP,
                                    DeliveryFulfillStatusType.REACHED_PICKUP,
                                    DeliveryFulfillStatusType.PICKED_UP)
                            .contains(currentStatus)) {
                        String riderLocationPickupStage = redisService
                                .getRedisData(Constants.REDIS_KEY_RIDER_LOCATION_PICKUP_STAGE)
                                .orElse("5");
                        expiryDuration = Duration.ofMinutes(Long.parseLong(riderLocationPickupStage));
                    } else if (List.of(
                                    DeliveryFulfillStatusType.OUT_FOR_DELIVERY,
                                    DeliveryFulfillStatusType.REACHED_DELIVERY)
                            .contains(currentStatus)) {
                        String riderLocationOfdStage = redisService
                                .getRedisData(Constants.REDIS_KEY_RIDER_LOCATION_OFD_STAGE)
                                .orElse("10");
                        expiryDuration = Duration.ofMinutes(Long.parseLong(riderLocationOfdStage));
                    } else {
                        log.info(
                                "Order ID: {} has reached final status: {}, stopping tracking.",
                                orderId,
                                currentStatus);
                        return;
                    }
