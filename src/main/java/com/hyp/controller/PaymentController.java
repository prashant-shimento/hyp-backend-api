...
public class PaymentController extends BaseListController<PaymentDto, Payment, String> {
    ...
    public ResponseEntity<Response> consumePayment(@PathVariable String orderId)
        ...
        return ResponseEntity.ok(Response.builder()
                ...
                .error(false)
                .message("Order Payment Consumed Successfully")
                .build());
    }

    @PostMapping("/process/{orderId}")
    public ResponseEntity<Response> processPayment(@PathVariable String orderId)
            throws PaymentException, EntityNotFoundException {
        Order order = Optional.ofNullable(orderService.findById(orderId))
                .orElseThrow(() -> new EntityNotFoundException("Order", orderId));
        Payment payment = Optional.ofNullable(paymentService.findByOrderId(orderId))
    ...
    private void handlePaymentSuccessEvent(RazorpayEventDto razorPayEventDto) throws PaymentException {
        String paymentOrderId =
                razorPayEventDto.getPayload().getPayment().getEntity().getOrder_id();
        ...
        if (razorPayEventDto.getPayload().getPayment() != null) {
            String paymentId =
                    razorPayEventDto.getPayload().getPayment().getEntity().getId();
            String status =
                    razorPayEventDto.getPayload().getPayment().getEntity().getStatus();
    ...
    private void handleRefundEvent(RazorpayEventDto razorPayEventDto, OrderStatusType orderStatus) {
        ...
        payment.getRefund()
                .setStatus(razorPayEventDto.getPayload().getRefund().getEntity().getStatus());
        paymentService.save(payment);
    }
}
