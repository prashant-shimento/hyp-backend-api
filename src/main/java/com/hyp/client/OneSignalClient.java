...
public class OneSignalClient {
	...
	public Mono<Void> registerUser(String userId, String oneSignalId) throws OneSignalException  {
		String endpoint = baseUrl + "/apps/" + appId + "/users/by/onesignal_id/" + oneSignalId + "/identity";
		...
		return executeWithRetry(getClient().patch().uri(endpoint)
				.contentType(MediaType.APPLICATION_JSON)
                .body(BodyInserters.fromValue(identityRequest))
				.retrieve()
				.bodyToMono(Void.class)
				.doOnSuccess(unused -> log.info("registerUser completed successfully for user: {}",
						userId))
				.doOnError(e -> log.error("Error in registerUser for user {}: {}", userId,
						e.getMessage(), e))
				.onErrorMap(e -> {
					log.error("Final failure after retries for user {}. Alerting about failure...",
							userId);
					OneSignalException oneSignalException = new OneSignalException("registerUser" + " " + userId, e.getMessage());
