...
public class DiscountRepositoryTest {
	...
	public void saveDiscount_Failure() {
		...
		}).isInstanceOf(IllegalArgumentException.class).hasMessageContaining("ID must not be null");
	}
	...
	public void updateDiscount_Success() {
		Discount discountToUpdate = discountRepository.findById(discount.getId())
		...
		Discount updatedDiscount = discountRepository.findById(discount.getId())
				.orElseThrow(() -> new RuntimeException(discount.getId() + " : not found"));
	...
	public void updateDiscount_Failure() {
		...
		Assertions.assertThatThrownBy(() -> {
			Discount discountToUpdate = discountToUpdateOpt
					.orElseThrow(() -> new RuntimeException("45655 : not found"));
			discountToUpdate.setActive("False");
			discountRepository.save(discountToUpdate);
		}).isInstanceOf(RuntimeException.class).hasMessageContaining("45655 : not found");
	}
